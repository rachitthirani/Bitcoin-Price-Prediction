...
def computeDelta(wt, X, Xi):
    ...
    num =0.0
    denom = 0.0
    mean_x = np.mean(X.iloc[0:-1])
    sd_x = np.std(X.iloc[0:-1])
    for i in range(0,len(Xi)):
        mean_xi = np.mean(Xi.iloc[i,0:-1])
        sd_xi = np.std(Xi.iloc[i,0:-1])
        similarity = 0.0
        for j in range(0,len(X)-1):
            similarity = similarity + ((X.iloc[j]-mean_x)*(Xi.iloc[i,j] - mean_xi))
        similarity = similarity / ((len(X)-1)*sd_x*sd_xi)
        num = num + (np.exp(wt*similarity) * Xi.iloc[i,(len(X)-1)])
        denom = denom + np.exp(wt*similarity)
    return num/denom
...
MSE = 0.0
# YOUR CODE HERE
MSE = sm.mean_squared_error(compareDF.iloc[0:len(compareDF),0],compareDF.iloc[0:len(compareDF),1])
print "The MSE is %f" % (MSE)

#references - http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html
